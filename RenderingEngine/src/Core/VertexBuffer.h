...
namespace OGE
{
	...
	public:
		...
		//vertices为要发送的顶点数据，length为indices的长度，size为缓存大小
		static SPtr(VertexBuffer) Create(const value_type* vertices, int length, int size = 0, Type type = STATIC_DRAW)
		{
	...
	protected:
		...
		VertexBuffer(const value_type* vertices, int length, int size = 0, Type type = STATIC_DRAW);
	};
