...
namespace OGE
{
    ...
    public:
        ...
        typedef Vec3f::value_type value_type;

        BoundingBoxf() :
            min_(FLT_MAX, FLT_MAX, FLT_MAX),
            max_(-FLT_MAX, -FLT_MAX, -FLT_MAX) {}

        BoundingBoxf(const BoundingBoxf& bb) :
            min_(bb.min_),
            max_(bb.max_) {}

        BoundingBoxf(const vec_type& min, const vec_type& max) :
            min_(min),
            max_(max) {}

        BoundingBoxf(value_type xmin, value_type ymin, value_type zmin,
        ...
        void Init()
        {
            ...
            max_.Set(-FLT_MAX, -FLT_MAX, -FLT_MAX);
        }
        ...
        //中心点
        vec_type Center() const { return (min_ + max_) * 0.5f; }

        //半径
        value_type Radius() const { return OGE::Sqrt(Radius2()); }

        //半径平方
        value_type Radius2() const { return  ((max_ - min_).Mod2()) * 0.25f; }
        ...
        void ExpandBy(const BoundingBoxf& bb)
        {
            if (!bb.IsValid())
                return;
        ...
        //返回两包围盒相交区域
        BoundingBoxf IntersectBox(const BoundingBoxf& bb) const
        {
            return BoundingBoxf(OGE::Maximum(min_.x(), bb.min_.x()), OGE::Maximum(min_.y(), bb.min_.y()), OGE::Maximum(min_.z(), bb.min_.z()),
                OGE::Minimum(max_.x(), bb.max_.x()), OGE::Minimum(max_.y(), bb.max_.y()), OGE::Minimum(max_.z(), bb.max_.z()));
        }

        //返回true表示两包围盒相交
        bool Isintersect(const BoundingBoxf& bb) const
        {
        ...
        BoundingBoxf& operator = (const BoundingBoxf& bb)
        {
            if (&bb == this)
                return *this;
            min_ = bb.min_;
        ...
        bool operator == (const BoundingBoxf& bb) const { return min_ == bb.min_ && max_ == bb.max_; }
